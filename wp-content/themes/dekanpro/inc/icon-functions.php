 ...
 */
function dekanpro_nav_menu_item_args( $args, $item, $depth ) {

	if ( dekanpro_option( 'main_nav_sub_indicators' ) ) {
		$dropdown_indicator = dekanpro()->icons->get_svg( 'chevron-down' );
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			if ( false === strpos( $args->link_after, $dropdown_indicator ) ) {
				$args->link_after .= $dropdown_indicator;
				$args->after       = '<button type="button" class="dekanpro-mobile-toggen">' . $dropdown_indicator . '</button>';
			}
		} else {
...
add_filter( 'nav_menu_item_args', 'dekanpro_nav_menu_item_args', 10, 3 );
 ...
 */
function dekanpro_nav_menu_social_icons( $item_output, $item, $depth, $args ) {

	// Get supported social icons.
	$social_icons = dekanpro_social_links_icons();

	// Change SVG icon inside social links menu if there is supported URL.
	if ( false !== strpos( $args->menu_class, 'dekanpro-socials-menu' ) ) {

		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace(
					$args->link_after,
					'</span><span class="' . esc_attr( $value ) . '" title="' . esc_attr( ucfirst( $value ) ) . '">' . dekanpro()->icons->get_svg( $value, array( 'aria-hidden' => 'true' ) ) . dekanpro()->icons->get_svg(
						$value,
						array(
							'class'       => 'bottom-icon',
							'aria-hidden' => 'true',
...
add_filter( 'walker_nav_menu_start_el', 'dekanpro_nav_menu_social_icons', 10, 4 );
 ...
 */
function dekanpro_social_links_icons() {
	...
	/**
	 * Filter Dekanpro social links icons.
	 *
	 * @since 1.0.0
	 * @param array $social_links_icons Array of social links icons.
	 */
	return apply_filters( 'dekanpro_social_links_icons', $social_links_icons );
}

if ( ! class_exists( 'Dekanpro_Icons' ) ) :
	/**
	 * Dekanpro Icons class.
	 *
	 * @since 1.0.0
	 */
	class Dekanpro_Icons {
		/**
		...
		public function get_svg( $icon = '', $args = array() ) {
			...
			// .dekanpro-icon is a required class.
			if ( false === strpos( $args['class'], 'dekanpro-icon' ) ) {
				$args['class'] = trim( 'dekanpro-icon ' . $args['class'] );
			}
		 ...
		 */
		public function get() {
			return apply_filters( 'dekanpro_icons_svg', self::$icons );
		}
		...
		public function get_meta_icon( $slug = '', $icon = '', $post_id = '' ) {
			...
			if ( is_single( $post_id ) ) {
				if ( dekanpro_option( 'single_entry_meta_icons' ) ) {
					$return = $icon;
				}
			} elseif ( ! is_single() ) {
				if ( dekanpro_option( 'entry_meta_icons' ) ) {
					$return = $icon;
				}
			}

			return apply_filters( 'dekanpro_' . $slug . '_meta_icon', $return, $post_id );
		}
