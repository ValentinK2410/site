 ...
 * @since 1.0.0
 */
class Dekanpro_Plugin_Utilities {
	...
	/**
	 * Main Dekanpro Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return Dekanpro_Plugin_Utilities
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dekanpro_Plugin_Utilities ) ) {
	...
	public function activate_plugin( $plugin ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error(
				...
				esc_html__( 'Current user can\'t activate plugins', 'dekanpro' )
			);
		}
		...
		if ( empty( $plugin ) ) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__( 'Missing plugin data.', 'dekanpro' )
			);
		}
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Plugin %s is not installed.', 'dekanpro' ),
					$plugin
				)
			);
		}
		...
		do_action( 'dekanpro_plugin_activated_' . $plugin );
	}
	...
	public function deactivate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__( 'Plugin %s is not active.', 'dekanpro' ),
					$plugin
				)
			);
		}
		...
		do_action( 'dekanpro_plugin_deactivated_' . $plugin );
	}
 ...
 *
 * Example: <?php $dekanpro_plugin_utilities = dekanpro_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function dekanpro_plugin_utilities() {
	return Dekanpro_Plugin_Utilities::instance();
}
