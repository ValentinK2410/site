...
if ( ! class_exists( 'Dekanpro_SocialSnap' ) ) :
	/**
	 * Social Snap compatibility class.
	 */
	class Dekanpro_SocialSnap {
		...
		public function __construct() {
			...
			add_filter( 'dekanpro_default_option_values', array( $this, 'default_customizer_values' ), 20 );

			// Remove Social Snap Lite from recommended plugins.
			add_filter( 'dekanpro_recommended_plugins', array( $this, 'update_recommended_plugins' ) );

			// Include helper functions.
			require DEKANPRO_THEME_PATH . '/inc/compatibility/socialsnap/socialsnap-functions.php'; // phpcs:ignore
		}
		...
		public function register_options( $options ) {
			...
			$options['setting']['dekanpro_single_post_meta_elements']['control']['choices']['shares'] = esc_html__( 'Shares', 'dekanpro' );

			$options['setting']['dekanpro_blog_entry_meta_elements']['control']['choices']['shares'] = esc_html__( 'Shares', 'dekanpro' );
		...
		public function default_customizer_values( $defaults ) {
			...
			$defaults['dekanpro_single_post_meta_elements']['shares'] = false;
			$defaults['dekanpro_blog_entry_meta_elements']['shares']  = false;
