...
if ( ! dekanpro_is_woocommerce_activated() ) {
	add_action( 'activate_woocommerce/woocommerce.php', array( dekanpro_dynamic_styles(), 'delete_dynamic_file' ) );
	return;
}

/**
 * Dekanpro WooCommerce Compatibility.
 */
if ( ! class_exists( 'Dekanpro_Woocommerce' ) ) :
	...
	class Dekanpro_Woocommerce {
		 ...
		 *
		 * @since 1.0.0
		 * @return Dekanpro_Woocommerce
		 */
		public static function instance() {

			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dekanpro_Woocommerce ) ) {
				self::$instance = new Dekanpro_Woocommerce();
		...
		private function includes() {
			...
			require DEKANPRO_THEME_PATH . '/inc/compatibility/woocommerce/woocommerce-functions.php'; // phpcs:ignore
			require DEKANPRO_THEME_PATH . '/inc/compatibility/woocommerce/class-dekanpro-customizer-woocommerce.php'; // phpcs:ignore
		}
		...
		private function actions() {
			...
			if ( ! is_admin() ) {
				...
				// Disable Dekanpro page description.
				add_filter( 'dekanpro_page_header_description', array( $this, 'shop_remove_page_description' ) );
				...
				// Extend Dekanpro breadcrumb trail.
				add_filter( 'dekanpro_breadcrumb_trail_items', array( $this, 'breadcrumbs' ), 20, 2 );
				add_filter( 'dekanpro_post_type_archive_title', array( $this, 'breadcrumb_post_type_archive_title' ), 10, 2 );

				// Remove WooCommerce sidebar.
				remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar' );
				add_action( 'dekanpro_woocommerce_sidebar', 'woocommerce_get_sidebar' );
				...
				// Replace WooCommerce pagination with Dekanpro pagination.
				remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
				add_action( 'woocommerce_after_shop_loop', 'dekanpro_pagination' );

				// Add back to shop button to Empty Cart.
				add_action( 'woocommerce_cart_is_empty', 'dekanpro_wc_empty_cart_button' );
				...
				add_filter( 'woocommerce_sale_flash', 'dekanpro_wc_add_percentage_to_sale_badge', 20, 3 );

				// Out of stock product badge.
				add_action( 'woocommerce_before_shop_loop_item_title', 'dekanpro_wc_out_of_stock_badge', 10 );
				add_action( 'woocommerce_before_single_product_summary', 'dekanpro_wc_out_of_stock_badge', 10 );
				...
				add_action( 'woocommerce_widget_shopping_cart_buttons', 'dekanpro_wc_widget_shopping_cart_buttons', 10 );
				...
				add_filter( 'woocommerce_subcategory_count_html', 'dekanpro_wc_cat_count_filter', 10, 2 );
				add_filter( 'woocommerce_rating_filter_count', 'dekanpro_wc_rating_count_filter', 10, 3 );
				add_filter( 'woocommerce_layered_nav_count', 'dekanpro_wc_layered_count_filter', 10, 3 );
			...
			// Enqueue styles.
			add_action( 'dekanpro_enqueue_scripts', array( $this, 'enqueue' ) );
			...
			// Add correct sidebar.
			add_filter( 'dekanpro_sidebar_name', array( $this, 'set_sidebar' ) );

			// Set sidebar position.
			add_filter( 'dekanpro_default_sidebar_position', array( $this, 'set_default_sidebar_position' ) );

			// Remove item from cart.
			add_action( 'wp_ajax_dekanpro_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );
			add_action( 'wp_ajax_nopriv_dekanpro_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );

			// Add theme supports.
			add_action( 'after_setup_theme', array( $this, 'theme_supports' ), 20 );

			// Add customizer cart widget.
			add_filter( 'dekanpro_customizer_widgets', array( $this, 'add_customizer_cart_widget' ) );
			add_filter( 'dekanpro_main_header_widgets', array( $this, 'add_cart_to_main_header_widgets' ) );
			...
			// Add dynamic CSS.
			add_filter( 'dekanpro_dynamic_styles', array( $this, 'dynamic_css' ), 5 );

			// Update dynamic styles on deactivation.
			add_action( 'deactivate_woocommerce/woocommerce.php', array( dekanpro_dynamic_styles(), 'delete_dynamic_file' ) );

			// Return Shop page ID.
			add_filter( 'dekanpro_get_the_id', array( $this, 'get_the_id' ) );
		...
		public function theme_supports() {
			...
			if ( dekanpro_option( 'wc_product_gallery_zoom' ) ) {
				add_theme_support( 'wc-product-gallery-zoom' );
			}

			// Product Gallery Lightbox.
			if ( dekanpro_option( 'wc_product_gallery_lightbox' ) ) {
		...
		public function enqueue() {
			...
			wp_enqueue_style(
				'dekanpro-woocommerce',
				DEKANPRO_THEME_URI . '/assets/css/compatibility/woocommerce' . $dekanpro_suffix . '.css',
				false,
			...
			wp_enqueue_script(
				'dekanpro-wc',
				DEKANPRO_THEME_URI . '/assets/js/' . $dekanpro_dir . 'dekanpro-wc' . $dekanpro_suffix . '.js',
				array( 'jquery' ),
				DEKANPRO_THEME_VERSION,
				true
			);
		...
		public function product_catalog_elements() {
			...
			$elements = dekanpro_option( 'product_catalog_elements' );
		...
		public function template_loop_category() {
			...
			if ( is_array( $product_cats ) && ! empty( $product_cats ) ) {
				foreach ( $product_cats as $product_cat ) {
					$cats[] = '<a class="dekanpro-loop-product__category" href="' . esc_url( get_term_link( $product_cat, 'product_cat' ) ) . '">' . esc_html( $product_cat->name ) . '</a>';
				}
			}

			echo '<span class="dekanpro-loop-product__category-wrap">' . wp_kses_post( implode( ', ', $cats ) ) . '</span>';
		}
		...
		public function result_wrapper_start() {
			...
			echo '<div class="dekanpro-woo-before-shop clearfix">';
		}
		...
		public function cart_widget_count_fragment( $fragments ) {
			...
			$fragments['.dekanpro-header-widget__cart a.dekanpro-cart'] = dekanpro_wc_cart_icon( false );
		...
		public function cart_widget_dropdown_fragment( $fragments ) {
			...
			$fragments['.dekanpro-header-widget__cart .dropdown-item'] = dekanpro_wc_cart_dropdown( false );
		...
		public function content_wrapper_end() {
			?>
				...
				<?php do_action( 'dekanpro_woocommerce_sidebar' ); ?>
		...
		public function single_product_wrapper_start() {
			echo '<div class="dekanpro-wc-product-wrap">';
		}
		...
		public function single_product_wrapper_end() {
			echo '</div><!-- END .dekanpro-wc-product-wrap -->';
		}
		...
		public function product_actions() {
			...
			// Remove Dekanpro page title on WooCommerce product pages.
			add_filter( 'dekanpro_page_header_has_title', '__return_false' );

			// Disable Comments toggle.
			add_filter( 'dekanpro_display_comments_toggle', '__return_false' );
		}
		...
		public function add_customizer_cart_widget( $widgets ) {
			...
			$widgets['cart'] = 'Dekanpro_Customizer_Widget_Cart';
		...
		public function register_wc_sidebars() {
			...
			register_sidebar(
				apply_filters(
					'dekanpro_woocommerce_sidebar_name',
					array(
						'name'          => esc_html__( 'WooCommerce Sidebar', 'dekanpro' ),
						'id'            => 'dekanpro-wc-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce pages except Product pages.', 'dekanpro' ),
						'before_widget' => '<div id="%1$s" class="dekanpro-sidebar-widget dekanpro-widget widget %2$s">',
						'after_widget'  => '</div>',
			...
			register_sidebar(
				apply_filters(
					'dekanpro_woocommerce_product_sidebar_name',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'dekanpro' ),
						'id'            => 'dekanpro-wc-product-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce Product pages.', 'dekanpro' ),
						'before_widget' => '<div id="%1$s" class="dekanpro-sidebar-widget dekanpro-widget widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h4 class="widget-title">',
						'after_title'   => '</h4>',
		...
		public function set_sidebar( $sidebar_name ) {
			...
			if ( is_product() ) {
				$sidebar_name = 'dekanpro-wc-product-sidebar';
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$sidebar_name = 'dekanpro-wc-sidebar';
			}
		...
		public function set_default_sidebar_position( $position ) {
			...
			if ( is_product() ) {
				$position = dekanpro_option( 'wc_product_sidebar_position' );
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$position = dekanpro_option( 'wc_sidebar_position' );
			}

			if ( is_product() || is_woocommerce() || is_cart() || is_checkout() ) {
				if ( 'default' === $position ) {
					return dekanpro_option( 'sidebar_position' );
				} else {
					return $position;
				}
			}
		...
		/**
		 * Remove Dekanpro page description on WooCommerce pages.
		 *
		 * @since 1.0.0
		...
		public function remove_item_from_cart() {
			...
			check_ajax_referer( 'dekanpro-nonce' );
		...
		public function product_image_swap() {
			...
			$hover_style = dekanpro_option( 'shop_product_hover' );
			...
			if ( 'image-swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo wp_kses_post( apply_filters( 'dekanpro_woocommerce_product_image_swap', wp_get_attachment_image( $attachment_id, $image_size, false, array( 'class' => 'show-on-hover' ) ) ) );
				}
			}
		}
		...
		public function loop_product_thumb_wrap_start() {
			...
			$class = 'dekanpro-product-thumb';

			if ( 'image-swap' === dekanpro_option( 'shop_product_hover' ) ) {
		...
		public function loop_product_thumb_wrap_end() {
			echo '</div><!-- END .dekanpro-product-thumb -->';
		}
		...
		public function load_customizer_widget() {
			...
			$path = DEKANPRO_THEME_PATH . '/inc/compatibility/woocommerce/class-dekanpro-customizer-widget-cart.php';
		...
		public function review_order_heading() {
			...
			echo wp_kses_post( apply_filters( 'dekanpro_review_order_heading', '<h3>' . __( 'Payment', 'dekanpro' ) . '</h3>' ) );
		}
		...
		public function checkout_shipping_heading() {
			...
			echo wp_kses_post( apply_filters( 'dekanpro_checkout_shipping_heading', '<h3>' . __( 'Shipping', 'dekanpro' ) . '</h3>' ) );
		}
		...
		public function single_product_related_products_args( $args ) {
			...
			$columns = intval( dekanpro_option( 'wc_related_columns' ) );
			$rows    = intval( dekanpro_option( 'wc_related_rows' ) );
		...
		public function woocommerce_cross_sell_display() {
			...
			if ( dekanpro_option( 'wc_cross_sell_products' ) ) {

				$rows = intval( dekanpro_option( 'wc_cross_sell_rows' ) );
		...
		public function woocommerce_upsell_display() {
			...
			if ( dekanpro_option( 'wc_upsell_products' ) ) {

				$columns = intval( dekanpro_option( 'wc_upsells_columns' ) );
				$rows    = intval( dekanpro_option( 'wc_upsells_rows' ) );
		...
		public function woocommerce_related_products() {
			...
			if ( dekanpro_option( 'wc_related_products' ) ) {
		...
		public function single_product_slider_options( $options ) {
			...
			if ( ! dekanpro_option( 'wc_product_slider_arrows' ) ) {
				return $options;
			}

			$options['directionNav'] = true;
			$options['prevText']     = dekanpro_animated_arrow( 'left', false );
			$options['nextText']     = dekanpro_animated_arrow( 'right', false );
		...
		public function related_products_heading( $heading ) {
			return __( 'Related Products', 'dekanpro' );
		}
		...
		public function admin_redirects() {
			...
			if ( false !== strpos( $current_page, 'dekanpro' ) ) {
		...
		public function dynamic_css( $css ) {
			...
			$css .= '
				...
				.woocommerce div.product .woocommerce-tabs ul.tabs li a {
					color: ' . dekanpro_hex2rgba( $content_text_color, 0.73 ) . ';
				}
			...
			$css .= '
				...
				.woocommerce .quantity .dekanpro-woo-minus,
				.woocommerce .quantity .dekanpro-woo-plus {
			...
			$css .= '
				...
				.woocommerce ul.products li.product .dekanpro-loop-product__category-wrap a:hover {
					color: ' . dekanpro_option( 'content_link_hover_color' ) . ';
				}
			';
			...
			// Background.
			$header_background = dekanpro_option( 'header_background' );

			if ( 'color' === $header_background['background-type'] && $header_background['background-color'] ) {
				$css .= '
					.dekanpro-header-widget__cart .dekanpro-cart .dekanpro-cart-count { 
			...
			// Headings.
			$css .= dekanpro_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title, .woocommerce #reviews #comments h2, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'headings_font' );

			$css .= dekanpro_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title', 'h2_font' );
			$css .= dekanpro_dynamic_styles()->get_typography_field_css( '.woocommerce #reviews #comments h2', 'h3_font' );
			$css .= dekanpro_dynamic_styles()->get_typography_field_css( '.woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'h4_font' );
...
if ( ! function_exists( 'dekanpro_woocommerce' ) ) :
	/**
	 * The function which returns the one Dekanpro_Woocommerce instance.
	 *
	 * @since 1.0.0
	 * @return object
	 */
	function dekanpro_woocommerce() {
		return Dekanpro_Woocommerce::instance();
	}
endif;

dekanpro_woocommerce();
