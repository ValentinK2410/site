...
if ( ! class_exists( 'Dekanpro_Elementor' ) ) :

	/**
	 * Elementor compatibility.
	 */
	class Dekanpro_Elementor {
		 ...
		 *
		 * @since 1.0.0
		 * @return Dekanpro_Elementor
		 */
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dekanpro_Elementor ) ) {
				self::$instance = new Dekanpro_Elementor();
			}
		...
		public function enqueue_editor_style() {
			...
			wp_enqueue_style(
				'dekanpro-elementor-editor',
				DEKANPRO_THEME_URI . '/assets/css/compatibility/elementor-editor-style' . $dekanpro_suffix . '.css',
				false,
		...
		public function setup_postdata() {
			...
			if ( ! dekanpro_enable_page_builder_compatibility() ) {
				return;
			}
			...
			$id = dekanpro_get_the_id();

			$setup = get_post_meta( $id, '_dekanpro_page_builder_setup', true );

			if ( isset( $post ) && empty( $setup ) && ( is_admin() || is_singular() ) && empty( $post->post_content ) && $this->is_built_with_elementor( $id ) ) {

				update_post_meta( $id, '_dekanpro_page_builder_setup', true );
				update_post_meta( $id, 'dekanpro_disable_page_title', true );
				update_post_meta( $id, 'dekanpro_disable_breadcrumbs', true );
				update_post_meta( $id, 'dekanpro_disable_thumbnail', true );
				update_post_meta( $id, 'dekanpro_sidebar_position', 'no-sidebar' );

				update_post_meta( $id, '_wp_page_template', 'page-templates/template-dekanpro-fullwidth.php' );
			}
		...
		public function enqueue_additional() {
			...
			wp_enqueue_style(
				'dekanpro-elementor',
				DEKANPRO_THEME_URI . '/assets/css/compatibility/elementor' . $dekanpro_suffix . '.css',
				false,
				DEKANPRO_THEME_VERSION,
				'all'
			);
...
/**
 * Returns the one Dekanpro_Elementor instance.
 */
function dekanpro_elementor() {
	return Dekanpro_Elementor::instance();
}
