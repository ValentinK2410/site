...
if ( ! class_exists( 'Dekanpro_Dashboard' ) ) :
	/**
	 * Dekanpro Dashboard page class.
	 */
	final class Dekanpro_Dashboard {
		...
		/**
		 * Main Dekanpro Dashboard Instance.
		 *
		 * @since 1.0.0
		 * @return Dekanpro_Dashboard
		 */
		public static function instance() {

			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dekanpro_Dashboard ) ) {
		...
		public function render_dashboard() {
			...
			<div class="hester-container">
				...
				<?php do_action( 'dekanpro_about_content_after' ); ?>
		...
		public function render_plugins() {
			...
			<div class="hester-container">
				...
				<?php do_action( 'dekanpro_recommended_plugins_after' ); ?>
		...
		public function render_changelog() {
			...
			$changelog = DEKANPRO_THEME_PATH . '/changelog.txt';

			if ( ! file_exists( $changelog ) ) {
				$changelog = esc_html__( 'Changelog file not found.', 'dekanpro' );
			} elseif ( ! is_readable( $changelog ) ) {
				$changelog = esc_html__( 'Changelog file not readable.', 'dekanpro' );
			} else {
			...
			<div class="hester-container">
				...
				<div class="hester-section-title">
					<h2 class="hester-section-title">
						<span><?php esc_html_e( 'DekanPro Theme Changelog', 'dekanpro' ); ?></span>
						<span class="changelog-version"><?php echo esc_html( sprintf( 'v%1$s', DEKANPRO_THEME_VERSION ) ); ?></span>
					</h2>
				...
				<?php do_action( 'dekanpro_after_changelog' ); ?>
							...
							<li class="<?php echo esc_attr( $current ); ?>">
								<a href="<?php echo esc_url( $item['url'] ); ?>">
									...
									if ( isset( $item['icon'] ) && $item['icon'] ) {
										dekanpro_print_admin_icon( $item['icon'] );
									}
									?>
								</a>
							</li>
		...
		/**
		 * Return the current Dekanpro Dashboard page.
		 *
		 * @since 1.0.0
		...
		public function get_current_page() {
			...
			$page = str_replace( 'dekanpro-', '', $page );
			$page = apply_filters( 'dekanpro_dashboard_current_page', $page );
		...
		public function get_navigation_items() {
			...
			return apply_filters( 'dekanpro_dashboard_navigation_items', $items );
		}
		...
		public function activate_plugin() {
			...
			// Security check.
			check_ajax_referer( 'dekanpro_nonce' );
			...
			if ( $plugin ) {
				...
				$response = dekanpro_plugin_utilities()->activate_plugin( $plugin );
		...
		public function deactivate_plugin() {
			...
			if ( $plugin ) {
				$response = dekanpro_plugin_utilities()->deactivate_plugin( $plugin );
		...
		public function highlight_submenu( $submenu_file ) {
			...
			// Check if we're on dekanpro plugins or changelog page.
			if ( 'themes.php' === $pagenow ) {
				if ( isset( $_GET['page'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( 'dekanpro-plugins' === $_GET['page'] || 'dekanpro-changelog' === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
						$submenu_file = 'dekanpro-dashboard';
					}
				}
			}
...
/**
 * The function which returns the one Dekanpro_Dashboard instance.
 *
 * Use this function like you would a global variable, except without needing
 * to declare the global.
 *
 * Example: <?php $dekanpro_dashboard = dekanpro_dashboard(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function dekanpro_dashboard() {
	return Dekanpro_Dashboard::instance();
}
