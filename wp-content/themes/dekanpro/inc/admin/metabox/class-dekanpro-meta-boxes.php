...
if ( ! class_exists( 'Dekanpro_Meta_Boxes' ) ) :

	/**
	 * Meta Boxes Class.
	 */
	class Dekanpro_Meta_Boxes {
		...
		public function metabox_config() {
			...
			// Sidebar position.
			$layout_settings['dekanpro_sidebar_position'] = array(
				'id'      => 'dekanpro_sidebar_position',
				'name'    => esc_html__( 'Sidebar', 'dekanpro' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''              => esc_html__( 'Default (from Customizer)', 'dekanpro' ),
			...
			// Content layout.
			$layout_settings['dekanpro_content_layout'] = array(
				'id'      => 'dekanpro_content_layout',
				'name'    => esc_html__( 'Content Layout', 'dekanpro' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''             => esc_html__( 'Default (from Customizer)', 'dekanpro' ),
			...
			$layout_settings['dekanpro_page_sections'] = array(
				'id'     => 'dekanpro_page_sections',
				'type'   => 'group',
				'fields' => array(),
			);

			// Disable Top Bar.
			if ( dekanpro_option( 'top_bar_enable' ) ) {
				$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_topbar'] = array(
					'id'      => 'dekanpro_disable_topbar',
					'name'    => esc_html__( 'Disable Top Bar', 'dekanpro' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Disable Header.
			$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_header'] = array(
				'id'      => 'dekanpro_disable_header',
				'name'    => esc_html__( 'Disable Main Header', 'dekanpro' ),
				'default' => false,
				'type'    => 'checkbox',
			);
			...
			if ( dekanpro_option( 'page_header_enable' ) ) {
				$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_page_title'] = array(
					'id'      => 'dekanpro_disable_page_title',
					'name'    => esc_html__( 'Disable Page Title', 'dekanpro' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( dekanpro_option( 'page_header_enable' ) && dekanpro_option( 'breadcrumbs_enable' ) && ! dekanpro_is_section_disabled( dekanpro_option( 'breadcrumbs_hide_on' ), get_the_ID() ) ) {
				$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_breadcrumbs'] = array(
					'id'      => 'dekanpro_disable_breadcrumbs',
					'name'    => esc_html__( 'Disable Breadcrumbs', 'dekanpro' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_thumbnail'] = array(
				'id'      => 'dekanpro_disable_thumbnail',
				'name'    => esc_html__( 'Disable Featured Image', 'dekanpro' ),
				'default' => false,
				'type'    => 'checkbox',
			);
			...
			if ( dekanpro_option( 'enable_footer' ) ) {
				$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_footer'] = array(
					'id'      => 'dekanpro_disable_footer',
					'name'    => esc_html__( 'Disable Main Footer', 'dekanpro' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( dekanpro_option( 'enable_copyright' ) ) {
				$layout_settings['dekanpro_page_sections']['fields']['dekanpro_disable_copyright'] = array(
					'id'      => 'dekanpro_disable_copyright',
					'name'    => esc_html__( 'Disable Copyright Bar', 'dekanpro' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Set metabox options.
			$this->metabox = array(
				'dekanpro-theme-page-options' => array(
					'id'         => 'dekanpro-theme-page-options',
					'post_type'  => $this->get_post_types(),
					'capability' => 'edit_posts',
					'label'      => esc_html__( 'Theme Page Options', 'dekanpro' ),
					'context'    => 'side',
					'priority'   => 'core',
			...
			$this->metabox = apply_filters( 'dekanpro_metabox_options', $this->metabox );
		}
		...
		public function render( $post, $metabox ) {
			...
			foreach ( $metabox['args']['settings'] as $setting ) {
				...
				echo '<div class="dekanpro-meta-box-field">';
		...
		public function render_field_checkbox( $post, $setting ) {
			...
			?>
			<div class="components-panel__row dekanpro-checkbox-row">
				<div class="components-base-control">
					<div class="components-base-control__field">
						<span class="components-checkbox-control__input-container dekanpro-checkbox-wrapper">
							...
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" class="dekanpro-checked-icon components-checkbox-control__checked" aria-hidden="true" focusable="false"><path d="M9 18.6L3.5 13l1-1L9 16.4l9.5-9.9 1 1z"></path></svg>
						</span>
		...
		public function load_assets( $hook ) {
			...
			if ( 'post-new.php' === $hook || 'post.php' === $hook ) {
				...
				if ( in_array( $post_type, $post_types, true ) ) {
					...
					wp_enqueue_style(
						'dekanpro-metabox-style',
						DEKANPRO_THEME_URI . '/inc/admin/assets/css/dekanpro-meta-boxes' . $dekanpro_suffix . '.css',
						false,
...
/**
 * The function which returns the one Dekanpro Meta Boxes instance.
 *
 ...
 * @return object
 */
function dekanpro_meta_boxes() {
	return Dekanpro_Meta_Boxes::get_instance();
}
