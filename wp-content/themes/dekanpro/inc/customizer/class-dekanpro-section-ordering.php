...
/**
 * Class Dekanpro_Section_Ordering
 */
class Dekanpro_Section_Ordering {
	...
	/**
	 * Main Dekanpro_Section_Ordering Instance.
	 *
	 * @since 1.0.0
	 * @return Dekanpro_Section_Ordering
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Dekanpro_Section_Ordering ) ) {
	...
	public function __construct() {
		...
		add_filter( 'dekanpro_section_priority', array( $this, 'get_section_priority' ), 10, 2 );
	}
	...
	public function get_section_priority( $value, $key = '' ) {
		...
		$orders = get_theme_mod( 'dekanpro_sections_order' );
		if ( empty( $orders ) ) {
			return $value;
		}
	...
	public function refresh_positions() {
		$section_order         = get_theme_mod( 'dekanpro_sections_order' );
		$section_order_decoded = json_decode( $section_order, true );
		if ( ! empty( $section_order_decoded ) ) {
			remove_all_actions( 'dekanpro_sections' );
	...
	private function hook_section_by_slug( $slug, $priority ) {
		...
		add_action( 'dekanpro_sections', $slug, absint( $priority ) );
	}
}


function dekanpro_section_reordering() {
	return Dekanpro_Section_Ordering::instance();
}
dekanpro_section_reordering();
