...
function DekanproLinkField($el) {
    ...
    const open = () => {
        ...
        var $textarea = jQuery('<textarea id="dekanpro-link-textarea" style="display:none;"></textarea>');
        if(!jQuery('#dekanpro-link-textarea').length)jQuery('body').append($textarea);

        var val = getNodeValue();
        wpLink.open('dekanpro-link-textarea', val.url, val.title, null);
    }
    ...
    const  onClose = () => {
        ...
        jQuery('#dekanpro-link-textarea').remove();
      }
...
( function( api, $ ) {
    api.controlConstructor['dekanpro-repeater'] = api.Control.extend({
        ready: function() {
            var control = this;
            setTimeout( function() {
                control._init();
        ...
        _init: function() {
            ...
            control.autocorrect_range_input_number = function( input_number, timeout ) {
                ...
                var range_input = input_number,
                    ...
                    reset = parseFloat( range.find( '.dekanpro-reset-range' ).attr( 'data-reset_value' ) ),
            ...
            if ( ! window._upload_dekanpro ) {
                ...
                window._upload_dekanpro = wp.media({
            ...
            window._upload_dekanpro.on( 'close', function() {

                // get selections and save to hidden input plus other AJAX stuff etc.
                var selection = window._upload_dekanpro.state().get( 'selection' );

            
            });
            window.media_current = {};
            window.media_btn = {};
            window._upload_dekanpro.on( 'select', function() {

                // Grab our attachment selection and construct a JSON representation of the model.
                var media_attachment = window._upload_dekanpro.state().get( 'selection' ).first().toJSON();
            ...
            control.handleGradient = function( $context ) {
                var control = this,
                    ...
                    range_input,
                    value,
                    dekanpro_range_input_number_timeout,
                ...
                $( '.item-gradient', $context ).each( function() {
                    ...
                    _item.find( '.dekanpro-range-wrapper' ).each( function() {
                        var $this = $( this );
                        ...
                        $this.find( 'input.dekanpro-range-input' ).on( 'change keyup', function() {
                            control.autocorrect_range_input_number( $( this ), 1000, dekanpro_range_input_number_timeout );
                        }).on( 'focusout', function() {
                            control.autocorrect_range_input_number( $( this ), 0, dekanpro_range_input_number_timeout );
                        });
                    ...
                    _item.on( 'change', '.dekanpro-select-wrapper', function() {
                        control.update_value();
                    });
            ...
            control.handleBackground = function( $context ) {
                ...
                $( '.item-design-options', $context ).each( function() {
                    ...
                    // Linked button
                    _item.on( 'click', '.dekanpro-spacing-linked', function() {
                    ...
                    // Unlinked button
                    _item.on( 'click', '.dekanpro-spacing-unlinked', function() {
            ...
            control.linkPicker = function( $context ){
                if(item_link = $context.find('.dekanpro-field-link')) {
                  const a =  new DekanproLinkField(item_link);
                  a.init();
                }
            }
            ...
            control.actions = function( $context ) {
                if ( control.params.live_title_id ) {
                    ...
                    if ( 'dekanpro_section_order_styling' === control.id ) {
...
jQuery( document ).ready( function( $ ) {
    ...
    var options_font_type = '',
        icon_group = '';
    $.each( Dekanpro_Icon_Picker.fonts, function( key, font ) {
        font = $.extend({}, {
            url: '',
            name: '',
            prefix: '',
    ...
    icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Dekanpro_Icon_Picker.search );
