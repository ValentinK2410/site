...
if ( ! class_exists( 'Dekanpro_Customizer_Control_Repeater' ) ) :

	/**
	 * Customizer Repeatable control class.
	 */
	class Dekanpro_Customizer_Control_Repeater extends Dekanpro_Customizer_Control {
		/**
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			if ( ! isset( $args['allow_unlimited'] ) || false !== $args['allow_unlimited'] ) {
				$this->max_item = apply_filters( 'dekanpro_reepeatable_max_item', absint( $args['max_item'] ) );
			} else {
				$this->max_item = absint( $args['max_item'] );
			}
			$this->changeable          = isset( $args['changeable'] ) && 'no' === $args['changeable'] ? 'no' : 'yes';
			$this->default_empty_title = isset( $args['default_empty_title'] ) && '' !== $args['default_empty_title'] ? $args['default_empty_title'] : esc_html__( 'Item', 'dekanpro' );

			add_action( 'customize_controls_print_footer_scripts', array( __CLASS__, 'item_tpl' ), 66 );
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'dekanpro_customize_controls_enqueue_scripts' ) );
		}
		...
		public function to_json() {
			...
			$this->json['add_text']            = $this->add_text ?? __( 'Add new item', 'dekanpro' );
			...
			$this->json['l10n'] = array(
				'image' => array(
					'placeholder'  => __( 'No image selected', 'dekanpro' ),
					'less'         => __( 'Less Settings', 'dekanpro' ),
					'more'         => __( 'Advanced', 'dekanpro' ),
					'select_image' => __( 'Select Image', 'dekanpro' ),
					'use_image'    => __( 'Use This Image', 'dekanpro' ),
				),
			);
		...
		public function enqueue() {
			...
			if ( in_array( 'button_set', array_values( array_column( $this->fields, 'type' ) ), true ) ) {
				wp_enqueue_style(
					'dekanpro-background-alignment-style',
					DEKANPRO_THEME_URI . '/inc/customizer/controls/alignment/alignment' . $dekanpro_suffix . '.css',
					false,
			...
			if ( in_array( 'background', array_values( array_column( $this->fields, 'type' ) ), true ) ) {
				wp_enqueue_style(
					'dekanpro-background-control-style',
					DEKANPRO_THEME_URI . '/inc/customizer/controls/background/background' . $dekanpro_suffix . '.css',
					false,
					DEKANPRO_THEME_VERSION,
					'all'
				);
		...
		<# if ( data.label ) {   #>
			<div class="dekanpro-control-heading customize-control-title dekanpro-field dekanpro-control-wrapper">
			...
			<div class="widget">
				<div class="widget-top">
					...
					<div class="widget-title">
						<h4 class="live-title"><?php esc_html_e( 'Item', 'dekanpro' ); ?></h4>
					</div>
				</div> 
									...
									#>
										<# if ( field.type !== 'checkbox' ) { #>
											<# if ( field.title && field.type != 'design-options' ) { #>
												<div class="dekanpro-control-heading dekanpro-control-wrapper">
													<div class="customize-control-title">
														...
														<# if ( field.desc ) { #>
															<i class="dekanpro-info-icon">
																...
																<span class="dekanpro-tooltip">{{{ field.desc }}}</span>
															</i>
														<# } #>
													</div>
												</div>
										...
										<# } else if(field.type === 'link') { #>
											...
											<div class="dekanpro-field dekanpro-field-link<# if ( field.value.url ) { #> -value<# } #> <# if ( field.value.target ) { #> -external<# } #>" data-name="link" data-type="link">
												<div class="dekanpro-input">
													<div class="dekanpro">
														
														<div class="dekanpro-hidden">
														...
														<a href="#" class="button" data-name="add" target=""><?php esc_html_e( 'Select Link', 'dekanpro' ); ?></a>
														...
														<div class="link-wrap">
															...
															<i class="dekanpro-icon -link-ext" title="Opens in a new window/tab"></i>
															<a class="dekanpro-icon -pencil -clear" data-name="edit" href="#" title="Edit"></a>
															<a class="dekanpro-icon -cancel -clear" data-name="remove" href="#" title="Remove"></a>
														</div>
													</div>
												</div>
											</div>
										...
										<# } else if ( field.type === 'button_set' ) { #>
											<div class="dekanpro-alignment-control button_set">
												<div class="button-group dekanpro-middle">
											<# for ( k in field.options ) { #>
												<# if ( field.options.hasOwnProperty( k ) ) { #>
													<label class="dekanpro-{{ k }}">
										...
										<# } else if ( field.type == 'media' ) { #>
											...
											<# } #>
											<div class="actions">
												<button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php esc_html_e( 'Remove', 'dekanpro' ); ?></button>
												<button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'dekanpro' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'dekanpro' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php esc_html_e( 'Add', 'dekanpro' ); ?> <# } else { #> <?php esc_html_e( 'Change', 'dekanpro' ); ?> <# } #> </button>
										...
										<# } else if ( field.type == 'icon'  ) { #>
											...
											<a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'dekanpro' ); ?></a>
											<# } else if(field.type == 'gradient'){ #>
												...
												<div data-dep-field="background-type" data-dep-value="gradient">
													...
													<div class="popup-element color-element style-1">
														<label for="gradient-color-1-{{ field.id }}"><?php esc_html_e( 'Color 1', 'dekanpro' ); ?></label>
													...
													<!-- Color 1 Location -->
													<div class="dekanpro-range-wrapper popup-element color-element style-2" data-option-id="gradient-color-1-location">
														<label for="gradient-color-1-location-{{ field.id }}"><?php esc_html_e( 'Color location', 'dekanpro' ); ?></label>

														<div class="dekanpro-control-wrap">
															<input 
																type="range" 
													...
													<!-- Color 2 -->
													<div class="popup-element color-element style-1">
														<label for="gradient-color-2-{{ field.id }}"><?php esc_html_e( 'Color 2', 'dekanpro' ); ?></label>
													...
													<!-- Color 2 Location -->
													<div class="dekanpro-range-wrapper popup-element color-element style-2" data-option-id="gradient-color-2-location">
														<label for="gradient-color-2-location-{{ field.id }}"><?php esc_html_e( 'Color location', 'dekanpro' ); ?></label>
													...
													<!-- Type -->
													<div class="dekanpro-select-wrapper popup-element style-1">
														<label for="gradient-type-{{ field.id }}"><?php esc_html_e( 'Gradient type', 'dekanpro' ); ?></label>
														<div class="popup-input-wrapper">
															<select data-option="gradient-type" id="gradient-type-{{ field.id }}" data-repeat-name="_items[__i__][{{ field.id }}][gradient-type]">
																<option value="linear"<# if ( 'linear' === field.value['gradient-type'] ) { #> selected="selected"<# } #>><?php esc_html_e( 'Linear', 'dekanpro' ); ?></option>
																<option value="radial"<# if ( 'radial' === field.value['gradient-type'] ) { #> selected="selected"<# } #>><?php esc_html_e( 'Radial', 'dekanpro' ); ?></option>
															</select>
														</div>
													</div>

													<!-- Linear Angle -->
													<div data-dep-field="gradient-type" data-dep-value="linear" class="dekanpro-range-wrapper popup-element color-element style-2" data-option-id="gradient-linear-angle">
														<label for="gradient-angle-{{ field.id }}"><?php esc_html_e( 'Angle', 'dekanpro' ); ?></label>
													...
													<!-- Radial Position -->
													<div data-dep-field="gradient-type" data-dep-value="radial" class="dekanpro-select-wrapper popup-element style-1">
														<label for="gradient-position-{{ field.id }}"><?php esc_html_e( 'Position', 'dekanpro' ); ?></label>
														<div class="popup-input-wrapper">
															<!-- <# var choices = 
															...
															<?php
																$choices = array(
																	'center center' => esc_html__( 'Center Center', 'dekanpro' ),
																	'center left' => esc_html__( 'Center Left', 'dekanpro' ),
																	'center right' => esc_html__( 'Center Right', 'dekanpro' ),
																	'top center' => esc_html__( 'Top Center', 'dekanpro' ),
																	'top left' => esc_html__( 'Top Left', 'dekanpro' ),
																	'top right' => esc_html__( 'Top Right', 'dekanpro' ),
																	'bottom center' => esc_html__( 'Bottom Center', 'dekanpro' ),
																	'bottom left' => esc_html__( 'Bottom Left', 'dekanpro' ),
																	'bottom right' => esc_html__( 'Bottom Right', 'dekanpro' ),
																);
																?>
												...
												<# } else if(field.type == 'design-options') { #> 
													<div class="dekanpro-design-options-wrapper dekanpro-popup-options dekanpro-control-wrapper">
													<div class="dekanpro-design-options-heading">
														<# if ( field.title ) { #>
													...
													<div class="hidden popup-content">
														...
														<# if ( 'background' in field.display ) { #>
															...
															<!-- Background Type -->
															<div class="dekanpro-select-wrapper popup-element style-1">
																<label for="background-type-{{ field.id }}"><?php esc_html_e( 'Background type', 'dekanpro' ); ?></label>
																<div class="popup-input-wrapper">
															...
															<# if ( 'color' in field.display['background'] ) { #>
																...
																<div data-dep-field="background-type" data-dep-value="color">
																	<!-- Background Color -->
																	<div class="popup-element color-element style-1">
																		<label for="background-color-{{ field.id }}"><?php esc_html_e( 'Background color', 'dekanpro' ); ?></label>
															...
															<# if ( 'gradient' in field.display['background'] ) { #>
																...
																<div data-dep-field="background-type" data-dep-value="gradient">
																	...
																	<!-- Radial Position -->
																	<div data-dep-field="gradient-type" data-dep-value="radial" class="dekanpro-select-wrapper popup-element style-1">
																		<label for="gradient-position-{{ field.id }}"><?php esc_html_e( 'Gradient position', 'dekanpro' ); ?></label>
																		<div class="popup-input-wrapper">
																			<?php
																				$choices = array(
																					'center center' => esc_html__( 'Center Center', 'dekanpro' ),
															...
															<# if ( 'image' in field.display['background'] ) { #>
																...
																<div class="dekanpro-background-wrapper" data-dep-field="background-type" data-dep-value="image">
																	...
																	<div class="background-image">
																		...
																		<div class="attachment-media-view background-image-upload">
																			...
																			<# } else { #>
																				<div class="placeholder"><?php esc_html_e( 'No image selected', 'dekanpro' ); ?></div>
																			<# } #>
																			...
																			<div class="actions">
																				...
																				<button class="button background-image-upload-remove-button<# if ( ! field.value['background-image'] ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'dekanpro' ); ?></button>

																				<button type="button" class="button background-image-upload-button"><?php esc_html_e( 'Select image', 'dekanpro' ); ?></button>

																				<a href="#" class="advanced-settings<# if ( ! field.value['background-image'] ) { #> hidden<# } #>">
																					<span class="message"><?php esc_html_e( 'Advanced', 'dekanpro' ); ?></span>
																	...
																	<div class="background-image-advanced">
																		...
																		<div class="background-repeat">
																			<select {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-option="background-repeat" data-repeat-name="_items[__i__][{{ field.id }}][background-repeat]">
																				<option value="no-repeat"<# if ( 'no-repeat' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'No Repeat', 'dekanpro' ); ?></option>
																				<option value="repeat"<# if ( 'repeat' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat All', 'dekanpro' ); ?></option>
																				<option value="repeat-x"<# if ( 'repeat-x' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Horizontally', 'dekanpro' ); ?></option>
																				<option value="repeat-y"<# if ( 'repeat-y' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Vertically', 'dekanpro' ); ?></option>
																			</select>
																		</div>
																		...
																		<div class="background-position">
																			...
																			<h4><?php esc_html_e( 'Background Position', 'dekanpro' ); ?></h4>

																			<div class="dekanpro-range-wrapper" data-option-id="background-position-x">
																				<span><?php esc_html_e( 'Horizontal', 'dekanpro' ); ?></span>
																				<div class="dekanpro-control-wrap">
																					<input 
																						type="range" 
																					...
																					<input 
																						...
																						value="{{ field.value['background-position-x'] }}"  />
																					<span class="dekanpro-range-suffix">%</span>
																				</div>
																			</div>

																			<div class="dekanpro-range-wrapper" data-option-id="background-position-y">
																				<span><?php esc_html_e( 'Vertical', 'dekanpro' ); ?></span>
																				<div class="dekanpro-control-wrap">
																					<input 
																						type="range"
																		...
																		<div class="background-size">
																			<h4><?php esc_html_e( 'Background Size', 'dekanpro' ); ?></h4>
																			<div class="buttonset">
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="cover" id="{{ field.id }}cover" <# if ( 'cover' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}cover"><?php esc_html_e( 'Cover', 'dekanpro' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="contain" id="{{ field.id }}contain" <# if ( 'contain' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}contain"><?php esc_html_e( 'Contain', 'dekanpro' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="auto" id="{{ field.id }}auto" <# if ( 'auto' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}auto"><?php esc_html_e( 'Auto', 'dekanpro' ); ?></label>
																				</input>
																			</div>
																		</div>
																		...
																		<div class="background-attachment">
																			<h4><?php esc_html_e( 'Background Attachment', 'dekanpro' ); ?></h4>
																			<div class="buttonset">
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" lass="switch-input screen-reader-text" type="radio" value="inherit" id="{{ field.id }}inherit" <# if ( 'inherit' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}inherit"><?php esc_html_e( 'Inherit', 'dekanpro' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="scroll" id="{{ field.id }}scroll" <# if ( 'scroll' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}scroll"><?php esc_html_e( 'Scroll', 'dekanpro' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="fixed" id="{{ field.id }}fixed" <# if ( 'fixed' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}fixed"><?php esc_html_e( 'Fixed', 'dekanpro' ); ?></label>
																				</input>
																			</div>
																		</div>
																		...
																		<div class="background-color-overlay popup-element color-element style-1">
																			...
																			<label for="background-color-overlay-{{ field.id }}"><h4><?php esc_html_e( 'Overlay Color', 'dekanpro' ); ?></h4></label>
														...
														<# if ( 'border' in field.display ) { #>
															...
															<# if ( 'width' in field.display['border'] && 'positions' in field.display['border'] ) { #>
																...
																<div class="customize-control-dekanpro-spacing popup-element style-2">
																		...
																		<ul class="active">
																			...
																			<# _.each( field.display['border']['positions'], function( title, id ){ #>
																				<li class="spacing-control-wrap spacing-input">
																					...
																					<span class="dekanpro-spacing-label">{{{ title }}}</span>
																				</li>
																			<# }); #>

																			<li class="spacing-control-wrap">
																				<div class="spacing-link-values">
																					<span class="dashicons dashicons-admin-links dekanpro-spacing-linked" data-element="{{ field.id }}" title="{{ data.title }}"></span>
																					<span class="dashicons dashicons-editor-unlink dekanpro-spacing-unlinked" data-element="{{ field.id }}" title="{{ data.title }}"></span>
																				</div>
																			</li>
																...
																<div class="dekanpro-select-wrapper popup-element style-1">
																	...
																	<div class="popup-input-wrapper">
																		<select data-option="border-style" data-live-id="{{{ field.id }}}" id="border-style-{{ field.id }}" data-repeat-name="_items[__i__][{{ field.id }}][border-style]">
																			<?php
																				$choices = array(
																					'solid' => esc_html__( 'Solid', 'dekanpro' ),
																					'dotted' => esc_html__( 'Dotted', 'dekanpro' ),
																					'dashed' => esc_html__( 'Dashed', 'dekanpro' ),
																				);
																				?>
																				<?php foreach ( $choices  as $key => $value ) { ?>
							...
							<div class="widget-control-actions">
								<div class="alignleft">
									<span class="remove-btn-wrapper">
										<a href="#" class="repeat-control-remove" title=""><?php esc_html_e( 'Remove', 'dekanpro' ); ?></a> |
									</span>
									<a href="#" class="repeat-control-close"><?php esc_html_e( 'Close', 'dekanpro' ); ?></a>
								</div>
								<br class="clear">
							</div>
						</div>
		 ...
		 */
		public function dekanpro_customize_controls_enqueue_scripts() {
			wp_localize_script(
				'customize-controls',
				'Dekanpro_Icon_Picker',
				apply_filters(
					'dekanpro_icon_picker_js_setup',
					array(
						'search' => esc_html__( 'Search', 'dekanpro' ),
						'fonts'  => array(
							'font-awesome' => array(
								// Name of icon.
								'name'   => esc_html__( 'Font Awesome', 'dekanpro' ),
