;( function( $ ) {
 	...
 	wp.customize.controlConstructor['dekanpro-range'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.find( '.dekanpro-range-wrapper' ).rangeControl({
		...
		save: function() {
			...
			if ( false === devices || devices === undefined || 0 == devices.length ) {
				value.value = parseFloat( this.container.find( '.dekanpro-range-input' ).val() );
			} else {
				for ( var device in devices ) {
					value[ device ] = parseFloat( this.container.find( '.control-responsive.' + device + ' .dekanpro-range-input' ).val() );
				}
			}

			if ( this.container.find( '.dekanpro-control-unit' ).length ) {
				value.unit = this.container.find( '.dekanpro-control-unit input:checked' ).val();
			}
	...
	( function( $, undef ) {
		...
		RangeControl = {
			...
			_create: function() {
				...
				self.unit = el.find( '.dekanpro-control-wrap' ).attr( 'data-unit' );
			...
			_addListeners: function() {
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'keyup input paste', '.dekanpro-range-input', function() {
				...
				self.element.on( 'click', 'input[type=range]', function() {
					...
					range = $( this );
					input = range.parent().children( '.dekanpro-range-input' );
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'click', '.dekanpro-reset-range', function() {

					var $el;

					self.element.find( '.dekanpro-reset-range' ).each( function( index, el ) {

						$el = $( el );

						// Reset range unit.
						self.unit = $el.data( 'reset_unit' );
						$el.closest( '.dekanpro-range-wrapper' ).find( '.dekanpro-control-unit input[type="radio"][value="' + $el.data( 'reset_unit' ) + '"]' ).click();
				 ...
				 * @since 1.0.0
				 */
				self.element.on( 'click', '.dekanpro-control-unit input', function() {

					var $unit = $( this );

					self.element.find( '.dekanpro-control-wrap input[type="range"]' ).each( function( index, el ) {
					...
					if ( 'px' === $unit.val() && ( 'rem' === self.unit || 'em' === self.unit ) ) {
						...
						self.element.find( 'input.dekanpro-range-input' ).each( function( index, el ) {
			...
			_initResponsiveControls: function() {
				...
				var self = this,
					el = self.element,
					wrap = el.find( '.dekanpro-control-wrap' );

				el.addClass( 'dekanpro-control-responsive' );

				// Populate responsive switcher.
				if ( el.find( '.customize-control-title' ).length ) {

					var $switcher = $( '<ul class="dekanpro-responsive-switchers"></ul>' );

					for ( var device in self.options.responsive ) {
						$switcher.append( '<li class="' + device + '"><span class="preview-' + device + '" data-device="' + device + '"><span class="dekanpro-tooltip small-tooltip">' + device + '</span><i class="' + self.options.responsive[device].icon + '"></i></span></li>' );
					}
			...
			_initUnit: function() {
				...
				var template = wp.template( 'dekanpro-control-unit' );
				...
				if ( 'object' === typeof self.options.unit ) {
					...
					$( '<span class="dekanpro-range-unit">' + self.options.unit + '</span>' ).insertAfter( wrap.find( '.dekanpro-range-input' ) );
				}

				wrap.parent().find( '.dekanpro-control-unit input:checked' ).click();
			}
		};

		// Register the color picker as a widget.
		$.widget( 'dekanpro.rangeControl', RangeControl );
	}( jQuery ) );
