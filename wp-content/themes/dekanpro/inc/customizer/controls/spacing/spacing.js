;(function($) {
 	...
 	wp.customize.controlConstructor['dekanpro-spacing'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			// Change unit.
			control.container.find( '.dekanpro-control-unit input[type="radio"]' ).on( 'change', function() {
				control.save_value();
			});
		...
		save_value: function() {
			...
			var new_val = {},
				...
				choice,
				device,
				units   = this.container.find( '.dekanpro-control-unit' );
