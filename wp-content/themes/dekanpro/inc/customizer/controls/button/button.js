;(function( $ ) {
	...
	wp.customize.controlConstructor['dekanpro-button'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.button', function(e){
				...
				if ( action ) {
					...
					var data = {
						_ajax_nonce: dekanpro_customizer_localized.wpnonce,
						action: action,
					};

			 		$.post( dekanpro_customizer_localized.ajaxurl, data, function(response) {
