;(function($) {
	...
	wp.customize.controlConstructor['dekanpro-widget'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.dekanpro-widget-container .widget-top', function(){
				$(this).closest( '.widget' ).toggleClass( 'dekanpro-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});
			
			// Minimize widget content when clicked on Done
			control.container.on( 'click', '.dekanpro-widget-container .widget-control-close', function() {
				$(this).closest( '.widget' ).toggleClass( 'dekanpro-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});

			// Show available widgets
			control.container.on( 'click', '.dekanpro-add-widget', function(e) {
				e.preventDefault();
				control.updateList();
			});
		 	...
		 	control.container.on( 'click', '.dekanpro-widget-edit-nav', function(){
		 		wp.customize.control( 'nav_menu_locations[' + $(this).closest( '.dekanpro-widget-nav-container' ).data( 'menu-location' ) + ']' ).focus();
		 		control.close();
		 	});
			...
			$( control.container ).find( '.dekanpro-widget-nav-container' ).each( function(){
			...
			wp.customize( 'nav_menu_locations[' + menu_location + ']', function( value ) {
				value.bind( function( newval ) {
					...
					if ( newval ) {
						...
						$container.addClass( 'dekanpro-widget-nav-has-menu' )
							.find( '.dekanpro-widget-nav-name' )
							.html( menu_name );
					} else {
						$container.removeClass('dekanpro-widget-nav-has-menu');
					}
		...
		updateList: function(){
			...
			if ( widgets ) {
				...
				// Hide all widgets.
				$( '#dekanpro-available-widgets-list .dekanpro-widget' ).hide().removeClass('disabled');

				// Display allowed widgets.
				$.each( widgets, function( index, el ) {

					widget = $( '#dekanpro-available-widgets-list #dekanpro-widget-tpl-dekanpro_customizer_widget_' + index );

					widget.show();

					if ( el.hasOwnProperty( 'max_uses' ) && el.max_uses > 0 && el.max_uses <= $(self.container).find('.dekanpro-widget-container [data-widget-type="' + index + '"]').length ) {
						widget.addClass('disabled');
					}
				});
			} else {
				// Show all widgets
				$( '#dekanpro-available-widgets-list .dekanpro-widget' ).show();
			}
		},
		...
		addWidget: function( widget_id_base ) {
			...
			// Get widget form
			widget_html = $.trim( $(this.container).find( '.dekanpro-widget-tpl-' + widget_id_base ).html() );
			widget_html = widget_html.replace( /<[^<>]+>/g, function( m ) {
				return m.replace( /__i__|%i%/g, widget_uuid );
			} );

			// Append new widget.
			var $widget = $( widget_html ).appendTo( this.container.find( '.dekanpro-widget-container' ) );
			...
			if ( $widget.find( '.dekanpro-widget-nav-container' ).length ) {
				this.bindMenuLocation( $widget.find( '.dekanpro-widget-nav-container' ) );
			}
		},

		close: function() {
			$( 'body' ).removeClass( 'dekanpro-adding-widget' );
		},

		update: function() {

			// Get all widgets in the area
			var widgets = this.container.find( '.dekanpro-widget-container .widget' );
		...
		setupSortable: function() {
			...
			$( this.container ).find( '.dekanpro-widget-container' ).sortable({
				items: '> .widget',
				handle: '.widget-top',
				intersect: 'pointer',
				axis: 'y',
 	...
 	$(document).ready( function(){
	 	...
	 	$( '.wp-full-overlay' ).on( 'click', '.dekanpro-add-widget, .dekanpro-close-widgets-panel', function(e) {
	 		e.preventDefault();

	 		$( 'body' ).toggleClass( 'dekanpro-adding-widget' );

	 		if ( $( this ).data( 'location-title' ) ) {
	 			control = wp.customize.control( $(this).data('control') );
	 			$( '#dekanpro-available-widgets' ).attr( 'data-control', control.params.id ).find( '.dekanpro-widget-caption' ).find( 'h3' ).html( $(this).data( 'location-title' ) );
	 		}
	 	});

	 	$( '.wp-full-overlay' ).on( 'click', '.customize-section-back', function(e) {
	 		$( 'body' ).removeClass( 'dekanpro-adding-widget' );
	 		$( '#dekanpro-available-widgets' ).removeAttr( 'data-control' );
	 	});

		// Add widget to widget control.
	 	$( '#dekanpro-available-widgets' ).on( 'click', '.dekanpro-widget', function(e) {

	 		// Get active control.
			control = wp.customize.control( $( '#dekanpro-available-widgets' ).attr('data-control') );
