<?php
/**
 * Dekanpro Customizer Widget control class.
 *
 * @package     Dekanpro
...
if ( ! class_exists( 'Dekanpro_Customizer_Control_Widget' ) ) :
	/**
	 * Dekanpro Customizer custom select control class.
	 */
	class Dekanpro_Customizer_Control_Widget extends Dekanpro_Customizer_Control {
		...
		public function to_json() {
			...
			if ( is_array( $this->widgets ) && ! empty( $this->widgets ) ) {
				...
				foreach ( $this->widgets as $widget_id => $args ) {
					...
					ob_start();
					?>
					<div data-widget-id="<?php echo esc_attr( $instance->id_base ); ?>" class="dekanpro-widget-tpl-<?php echo esc_attr( $instance->id_base ); ?> dekanpro-widget" data-max-uses=<?php echo esc_attr( $max_uses ); ?>>
		...
		protected function content_template() {
			?>
			<div class="dekanpro-control-wrapper dekanpro-widget-wrapper">

				<div class="dekanpro-widget-container">

					<# _.each( data.value, function( template, i ) { #>
						{{{ template }}}
					<# }); #>

				</div><!-- END .dekanpro-widget-container -->

				<div class="dekanpro-add-widget-wrap">			
					<button type="button" class="button dekanpro-add-widget" data-location-title="{{{ data.label }}}" data-control="{{{ data.id }}}"><?php esc_html_e( 'Add Widget', 'dekanpro' ); ?></button>
				</div>

				<div class="dekanpro-widget-tmpls">
