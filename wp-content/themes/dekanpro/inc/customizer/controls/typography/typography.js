;( function( $ ) {
 	...
 	// Typography control
	wp.customize.controlConstructor[ 'dekanpro-typography' ] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.find( '.reset-defaults' ).on( 'click', function() {
				...
				rangeFields.forEach( ( item ) => {
					control.container.find( '[data-option-id="' + item + '"]' ).find( '.dekanpro-reset-range' ).click();
				});
			});
		...
		update_font_weight_field: function( font_family ) {
			...
			if ( 'inherit' === font_family || 'default' === font_family ) {
				...
				$.each( dekanpro_typography_vars.fonts, function( group_id, group ) {
			...
			self.container.find( '.dekanpro-typography-font-weight' ).find( 'select' ).html( options );
		},
		...
		update_font_subsets_field: function( font_family ) {
			...
			var self     = this,
				...
				selected = '',
				setting  = self.setting.get(),
				field    = self.container.find( '.dekanpro-typography-font-subsets' );
