<?php
/**
 * Dekanpro Customizer design options control class.
 *
 * @package DekanPro
 * @author DekanPro
 * @since   1.0.0
...
if ( ! class_exists( 'Dekanpro_Customizer_Control_Design_Options' ) ) :
	/**
	 * Dekanpro Customizer design options control class.
	 */
	class Dekanpro_Customizer_Control_Design_Options extends Dekanpro_Customizer_Control {
		 ...
		 * @var string
		 */
		public $type = 'dekanpro-design-options';
		...
		public function to_json() {
			...
			$this->json['l10n'] = array(
				'background-type' => esc_html__( 'Type', 'dekanpro' ),
				'gradient'        => array(
					'color-1'         => esc_html__( 'Color 1', 'dekanpro' ),
					'color-2'         => esc_html__( 'Color 2', 'dekanpro' ),
					'color-location'  => esc_html__( 'Location', 'dekanpro' ),
					'type'            => esc_html__( 'Type', 'dekanpro' ),
					'linear'          => esc_html__( 'Linear', 'dekanpro' ),
					'radial'          => esc_html__( 'Radial', 'dekanpro' ),
					'angle'           => esc_html__( 'Angle', 'dekanpro' ),
					'position'        => esc_html__( 'Position', 'dekanpro' ),
						...
						<# if ( 'color' in data.display['background'] ) { #>
							...
							<div data-dep-field="background-type" data-dep-value="color">
								...
								<div class="popup-element color-element style-1">
									...
									<input class="dekanpro-color-control" data-option="background-color" type="text" value="{{data.value['background-color']}}" data-show-opacity="true" data-default-color="{{data.default['background-color']}}" />
								</div>
							</div>
						<# } #>
						...
						<# if ( 'gradient' in data.display['background'] ) { #>
							...
							<div data-dep-field="background-type" data-dep-value="gradient">
								...
								<div class="popup-element color-element style-1">
									...
									<input class="dekanpro-color-control" data-option="gradient-color-1" type="text" value="{{data.value['gradient-color-1']}}" data-show-opacity="true" data-default-color="{{data.default['gradient-color-1']}}" />
								</div>
								...
								<div class="popup-element color-element style-1">
									...
									<input class="dekanpro-color-control" data-option="gradient-color-2" type="text" value="{{data.value['gradient-color-2']}}" data-show-opacity="true" data-default-color="{{data.default['gradient-color-2']}}" />
								</div>
								...
								<div class="background-image">
									...
									<div class="attachment-media-view background-image-upload">
										...
										<div class="actions">
											...
											<button class="button background-image-upload-remove-button<# if ( ! data.value['background-image'] ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'dekanpro' ); ?></button>
								...
								<div class="background-image-advanced">
									...
									<div class="background-repeat">
										<select {{{ data.inputAttrs }}} data-option="background-repeat">
											<option value="no-repeat"<# if ( 'no-repeat' === data.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'No Repeat', 'dekanpro' ); ?></option>
											<option value="repeat"<# if ( 'repeat' === data.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat All', 'dekanpro' ); ?></option>
											<option value="repeat-x"<# if ( 'repeat-x' === data.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Horizontally', 'dekanpro' ); ?></option>
											<option value="repeat-y"<# if ( 'repeat-y' === data.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Vertically', 'dekanpro' ); ?></option>
										</select>
									</div>
									...
									<div class="background-size">
										...
										<div class="buttonset">
											<input {{{ data.inputAttrs }}} data-option="background-size" class="switch-input screen-reader-text" type="radio" value="cover" name="_customize-bg-{{{ data.id }}}-size" id="{{ data.id }}cover" <# if ( 'cover' === data.value['background-size'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}cover"><?php esc_html_e( 'Cover', 'dekanpro' ); ?></label>
											</input>
											<input {{{ data.inputAttrs }}} data-option="background-size" class="switch-input screen-reader-text" type="radio" value="contain" name="_customize-bg-{{{ data.id }}}-size" id="{{ data.id }}contain" <# if ( 'contain' === data.value['background-size'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}contain"><?php esc_html_e( 'Contain', 'dekanpro' ); ?></label>
											</input>
											<input {{{ data.inputAttrs }}} data-option="background-size" class="switch-input screen-reader-text" type="radio" value="auto" name="_customize-bg-{{{ data.id }}}-size" id="{{ data.id }}auto" <# if ( 'auto' === data.value['background-size'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}auto"><?php esc_html_e( 'Auto', 'dekanpro' ); ?></label>
											</input>
										</div>
									</div>
									...
									<div class="background-attachment">
										...
										<div class="buttonset">
											<input {{{ data.inputAttrs }}} data-option="background-attachment" lass="switch-input screen-reader-text" type="radio" value="inherit" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}inherit" <# if ( 'inherit' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}inherit"><?php esc_html_e( 'Inherit', 'dekanpro' ); ?></label>
											</input>
											<input {{{ data.inputAttrs }}} data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="scroll" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}scroll" <# if ( 'scroll' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}scroll"><?php esc_html_e( 'Scroll', 'dekanpro' ); ?></label>
											</input>
											<input {{{ data.inputAttrs }}} data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="fixed" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}fixed" <# if ( 'fixed' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
												<label class="switch-label" for="{{ data.id }}fixed"><?php esc_html_e( 'Fixed', 'dekanpro' ); ?></label>
											</input>
										</div>
									</div>
									...
									<div class="background-color-overlay popup-element color-element style-1">
										...
										<label for="background-color-overlay-{{ data.id }}"><h4><?php esc_html_e( 'Overlay Color', 'dekanpro' ); ?></h4></label>

										<input class="dekanpro-color-control" data-option="background-color-overlay" type="text" value="{{data.value['background-color-overlay']}}" data-show-opacity="true" data-default-color="{{data.default['background-color-overlay']}}" />
									</div>
					...
					<# if ( 'color' in data.display ) { #>
						...
						<# _.each( data.display['color'], function( title, id ){ #>
							...
							<div class="popup-element color-element style-1">
								...
								<input class="dekanpro-color-control" data-option="{{ id }}" type="text" value="{{data.value[ id ]}}" data-show-opacity="true" data-default-color="{{data.default[ id ]}}" />
							</div>
						<# }); #>
					...
					<# if ( 'border' in data.display ) { #>
									...
									<ul class="active">
										...
										<li class="spacing-control-wrap">
											<div class="spacing-link-values">
												<span class="dashicons dashicons-admin-links dekanpro-spacing-linked" data-element="{{ data.id }}" title="{{ data.title }}"></span>
												<span class="dashicons dashicons-editor-unlink dekanpro-spacing-unlinked" data-element="{{ data.id }}" title="{{ data.title }}"></span>
											</div>
										</li>
						...
						<# if ( 'color' in data.display['border'] ) { #>
							...
							<div class="popup-element color-element style-1">
								...
								<input class="dekanpro-color-control" data-option="border-color" type="text" value="{{data.value['border-color']}}" data-show-opacity="true" data-default-color="{{data.default['border-color']}}" />
							</div>
						<# } #>
						...
						<# if ( 'separator' in data.display['border'] ) { #>
							...
							<div class="popup-element color-element style-1">
								...
								<input class="dekanpro-color-control" data-option="separator-color" type="text" value="{{data.value['separator-color']}}" data-show-opacity="true" data-default-color="{{data.default['separator-color']}}" />
							</div>
						<# } #>
