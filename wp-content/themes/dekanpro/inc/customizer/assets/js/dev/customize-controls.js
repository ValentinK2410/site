; ( function( api, $ ) {
	...
	// Extends our custom "dekanpro-info" section. Make it visible.
	api.sectionConstructor['dekanpro-info'] = api.Section.extend({
	...
	api.dekanproCustomizerPreviewer = {
		init: function() {
			var self = this,
				control;
	...
	// Store old previewer.
	var dekanproOldPreviewer = api.Previewer;
	api.Previewer = dekanproOldPreviewer.extend({
		initialize: function( params, options ) {

			// Store a reference to the Previewer
			api.dekanproCustomizerPreviewer.preview = this;

			// Call the old Previewer's initialize function
			dekanproOldPreviewer.prototype.initialize.call( this, params, options );
		}
	});

	api( 'dekanpro_info_style', function( setting ) {
		...
		setting.bind( function( value ) {
			if ( 12 == value ) {
				...
				api.control( 'dekanpro_info_section_spacing' ).setting( data );

				// console.log( 'New value', api.control( 'dekanpro_info_section_spacing' ).setting() );
			} else {
				var data = {
			...
			var selector = api.control( 'dekanpro_info_section_spacing' ).selector;
	...
	_.each( dekanpro_customizer_localized.preview_url_for_section, function( url, id ) { // eslint-disable-line camelcase
		if ( url ) {
	...
	$( document ).ready( function( $ ) {
		...
		api.dekanproCustomizerPreviewer.init();

		// Display the first responsive control
		$( '.dekanpro-control-responsive' ).each( function() {
			$( this ).find( '.control-responsive' ).first().addClass( 'active' );
		});

		// Responsive switchers
		$( '.customize-control' ).on( 'click', '.dekanpro-responsive-switchers span', function( event ) {

			var $this = $( this ),
				$switcherContainer = $this.closest( '.dekanpro-responsive-switchers' ),
				$switcherButtons = $switcherContainer.find( 'li span' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.dekanpro-control-responsive' ),
		...
		$( '.wp-full-overlay-footer .devices button' ).on( 'click', function( event ) {
			...
			// Set up variables
			var $this = $( this ),
				$devices = $( '.customize-control .dekanpro-responsive-switchers' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.dekanpro-control-responsive' );
        ...
        setTimeout(() => {
             // Tooltip positioning
            if ( $( '.dekanpro-tooltip' ).length ) {
                
                var $tooltip, $iconPosLeft, $iconPosRight, $titleWidth;

                $( '.dekanpro-tooltip' ).each( function() {
                    $tooltip = $( this );
                    ...
                    $titleWidth = $tooltip.closest( '.dekanpro-control-wrapper' ).outerWidth();

                    $iconPosLeft = $tooltip.closest( '.dekanpro-info-icon' ).css( 'position', 'static' ).position().left;
		...
		api.hasOwnProperty( 'previewer' ) && ( api.previewer.bind( 'dekanpro-customize-disable-section', function( e ) {
			$( 'input[id=' + e + ']' ).trigger( 'click' ).trigger( 'change' );
			const val = ! api.control( e ).setting.get();
			api.control( e ).setting.set( val );
		}), api.previewer.bind( 'dekanpro-customize-focus-control', function( e ) {
			api.control( e ).focus();
		}) );

		//Scroll to section
		$( 'body' ).on( 'click', '#sub-accordion-panel-dekanpro_panel_homepage > .accordion-section.control-subsection', function( event ) {
