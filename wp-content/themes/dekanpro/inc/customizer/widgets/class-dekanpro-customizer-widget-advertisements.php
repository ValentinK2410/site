...
if ( ! class_exists( 'Dekanpro_Customizer_Widget_Advertisements' ) ) :

	/**
	 * Dekanpro Customizer widget class
	 */
	class Dekanpro_Customizer_Widget_Advertisements extends Dekanpro_Customizer_Widget {
		...
		public function __construct( $args = array() ) {
			$args['name'] = __( 'Advertisement', 'dekanpro' );
		...
		public function form() {
			...
			<div class="banner-controls" id="bh-banner-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>">
				...
				<div class="banner-wrapper">
					<p>
						...
						<button class="button button-primary widget-media-upload <?php echo $this->values['image_id'] == 0 ? 'show' : 'hide'; ?>" data-widget-id="<?php echo esc_attr( $this->id ); ?>" data-widget-number="<?php echo esc_attr( $this->number ); ?>"><?php esc_html_e( 'Upload Banner', 'dekanpro' ); ?></button>
						...
						<button class="button button-secondary remove-image <?php echo $this->values['image_id'] !== 0 ? 'show' : 'hide'; ?>">&times;</button>
					</p>
				</div>

				<!-- URL -->
				<p>
					<label for="widget-ad-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-url"><?php esc_html_e( 'URL', 'dekanpro' ); ?>:</label>
					<input 
						type="text" 
						...
						placeholder="<?php esc_attr_e( 'Banner URL', 'dekanpro' ); ?>" />
				</p>

				<!-- Target -->
				<p>
					<label for="widget-ad-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-target"><?php esc_html_e( 'Open link in', 'dekanpro' ); ?>:</label>
					<span class="buttonset">
						<input 
							...
							<label 
								...
								<?php esc_html_e( 'Same Tab', 'dekanpro' ); ?>
							</label>
						</input>
						<input 
							class="switch-input screen-reader-text" 
							...
							<label 
								...
								<?php esc_html_e( 'New Tab', 'dekanpro' ); ?>
							</label>
						</input>
					</span>
				</p>
			...
			<div id="bh-ad-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>" class="dekanpro-checkbox-group-control dekanpro-ad-display-area">
				<div class="dekanpro-control-heading customize-control-title dekanpro-field">
					<span><?php esc_html_e( 'Show on:', 'dekanpro' ); ?> </span>			
				</div>
				<?php
				foreach ( $this->display_areas as $key => $value ) {
					...
					<p>
						<label class="dekanpro-checkbox">
							...
							<span class="dekanpro-label"><?php echo esc_html( $value ); ?></span>
						</label>
					</p>
				<?php } ?>
